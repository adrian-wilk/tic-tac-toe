...
public class Game {
    ...
    public String checkGameWinner(char [][]grid) {
        ...
        for (int i = 0; i < symbol.length; i++) {
            ...
            // This will check for 3 possible rows or columns
            for (int j = 0; j < grid.length; j++) {

                // This will check all three possible row wins.
                if (grid[0][j] == symbol[i] && grid[1][j] == symbol[i] && grid[2][j] == symbol[i]) {
                    return winner[i];
                }
                // This will check all three possible columns wins.
                if (grid[j][0] == symbol[i] && grid[j][1] == symbol[i] && grid[j][2] == symbol[i]) {
                    return winner[i];
                }
            }
            // This will check both possible diagonal wins.
